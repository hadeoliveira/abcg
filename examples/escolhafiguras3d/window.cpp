...
void Window::drawFigures() {
  ...
  // Create program
  m_program =
      abcg::createOpenGLProgram({{.source = assetsPath + "esfera.vert",
                                  .stage = abcg::ShaderStage::Vertex},
                                 {.source = assetsPath + "esfera.frag",
                                  .stage = abcg::ShaderStage::Fragment}});

  // Load model
  if (m_figuras == figuras3D::Esfera) {
    loadModelFromFile(assetsPath + "esfera.obj");
    standardize();
  } else if (m_figuras == figuras3D::Coelho) {
    loadModelFromFile(assetsPath + "bunny.obj");
    standardize();
  } else if (m_figuras == figuras3D::Bule) {
...
void Window::onPaintUI() {
  ...
  // Create a window for the other widgets
  {
    ...
    // Create a widget to choose a
    {
      static std::size_t index{};
      std::vector<std::string> const figuras_combo{"Esfera", "Coelho", "Bule"};

      ImGui::PushItemWidth(70);
      if (ImGui::BeginCombo("Figuras", figuras_combo.at(index).c_str())) {
        for (auto const index_select : iter::range(figuras_combo.size())) {
          auto const select{index == index_select};
          if (ImGui::Selectable(figuras_combo.at(index_select).c_str(), select))
            index = index_select;
          if (select)
            ImGui::SetItemDefaultFocus();
        }
        ...
        if (index == 0) {
          ...
          m_figuras = figuras3D::Coelho;
        } else {
          m_figuras = figuras3D::Bule;
        }
      }
    }
